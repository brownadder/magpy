...
def batch_first_term(H, tlist):
    ...
    return torch.tensordot(a, torch.stack([p() for p in H.pauli_operators()]), 1)


def batch_second_term(H, tlist):
    ...
    n = len(tlist) - 1
    commutators = torch.stack([__eval_commutator(H, tlist, i, j, n) for i, j in knot_slice_indices])
    
    return weights_second_term_coeff * torch.sum(commutators * weights_second_term, 0)


def __eval_commutator(H, tlist, i, j, n):
    ...
    s = torch.stack([p() for p in H.pauli_operators()])
